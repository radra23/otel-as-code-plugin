...
- semconv-lint is advisory by default (exit 0); **strict mode** hard-blocks (exit 2) on *severe* violations only — service.*-as-span-attr + deprecated http.method/http.url/http.status_code (Rules 1-4, exact deterministic literal matches); heuristic/judgment rules (5-8) stay warn-only, including Rule 8 (a high-cardinality identifier used as a metric-instrument dimension) — CONCEPTUALLY the worst-severity finding this hook can make, per semconv-discipline's cardinality-position ranking, but its detection is a same-file window heuristic that can't bind an `.Add()`/`.Record()` call to the specific instrument a constructor created, only to "some constructor exists somewhere in this file" — a real false positive was found and fixed post-merge, so it doesn't clear the bar strict-mode blocking requires. Opt in via `OTEL_STRICT=1` (env, e.g. CI) or a `.claude/.otel-strict` sentinel (same dual pattern as `--force`). Codex PostToolUse can't deny, so its adapter reframes the block as a must-fix `additionalContext`. The hook gates on file *content* referencing an OTel API (not filename — a basename-only gate skipped application code entirely, #138), and its attribute-setter matching is language-aware across all six supported languages, not just the JS/Java `setAttribute(` spelling.
...
## Conventions
- **Subagents have no `Skill` tool** and cannot read a skill by name (skills live in the plugin cache, off the target repo, with a path the agent doesn't know). A command that dispatches a subagent MUST hand it the guidance as explicit inputs — a value (e.g. `semconvVersion`) and/or an absolute `SKILL.md` path the subagent `Read`s (`semconvGuidancePath`, `languageMaturityPath`, `terraformPatternsPath`, resolved via `${CLAUDE_PLUGIN_ROOT}`). NEVER write "read the `otel-as-code:<skill>` skill" in an agent file — it silently fails (the scanner then stamped `guidanceVersion: null`, the auditor a remembered `1.27.0`). Only the main agent running a command has `Skill`.
