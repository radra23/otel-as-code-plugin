...
expect_blocked() {
  ...
  echo "existing content" > "$tmp/$name"
  ...
  if [ "$rc" -eq 1 ]; then
    ...
    echo "FAIL: protects existing $name (expected 1, got $rc)"; FAIL=$((FAIL+1))
  fi
  rm -rf "$tmp"
}
...
force_shape() {
  ...
  echo "existing content" > "$tmp/api/src/tracing.ts"
...
force_shape_blocks() {
  ...
  if [ "$rc" -eq 1 ]; then
    ...
    echo "FAIL: --force still blocks $label (expected 1, got $rc)"; FAIL=$((FAIL+1))
  fi
  rm -rf "$tmp"
}
