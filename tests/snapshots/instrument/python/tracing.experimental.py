# OpenTelemetry SDK bootstrap (--experimental) — generated by otel-as-code v0.1.0
# Semconv version: 1.44.0
# Re-run /otel-instrument --experimental to regenerate.
#
# This is the `--experimental` variant of tracing.py: identical traces + metrics wiring, PLUS the
# Development-level Python logs pipeline (opentelemetry.sdk._logs — an underscore/experimental
# module, gated behind --experimental; see skills/language-maturity/SKILL.md). The E2E harness
# seeds this file to prove the generated logs path actually exports OTLP log records.
import logging
import os
...
# Logs — EXPERIMENTAL. The Python logs signal is still experimental in the implementation
# (opentelemetry.sdk._logs is an underscore module and may change), even though the spec marks
# the Logs SDK stable.
...
# --- Exporter selection --------------------------------------------------------------
# OTEL_TRACES_EXPORTER / OTEL_METRICS_EXPORTER / OTEL_LOGS_EXPORTER select `otlp`, `console` or
# `none`. Building the providers by hand (rather than via `opentelemetry-instrument`) means
# honouring them here. The default is `none` when no endpoint is configured: defaulting to otlp
# would point at localhost:4317, dropping every span while a reconnect loop burns CPU. `console`
# prints to stdout with no collector running.
...
# Logs — EXPERIMENTAL (requires --experimental). Console uses Simple export so records print as
# they arrive; otlp batches. The stdlib root logger is bridged to OTel so existing `logging`
# calls export as OTLP log records.
