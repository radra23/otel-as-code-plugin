...
1. Rebuild the candidate identity-input list:
   ...
   { git ls-files; git ls-files --others --exclude-standard; } \
     | grep -E '(^|/)(package\.json|pnpm-workspace\.yaml|pyproject\.toml|requirements\.txt|go\.mod|go\.work|Cargo\.toml|pom\.xml|build\.gradle(\.kts)?|settings\.gradle(\.kts)?|global\.json|Directory\.Packages\.props|[^/]+\.(csproj|fsproj|sln)|Dockerfile|host\.json|serverless\.yml|CODEOWNERS)$' \
     | sort
   ```
