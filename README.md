...
Instrument any Node.js, Python, Java, .NET, or Go service, infer service identity from your repo,
...
**MVP / experimental.** Scope: Node.js, Python, Java, .NET, and Go instrumentation, and Terraform
...
releases weekly (see the drift check). The Node.js, Python, Java, .NET, and Go instrumentation is
additionally **proven end-to-end in CI**: the generated SDK bootstrap (for Java, the OpenTelemetry
Java agent; for .NET, the generated `OpenTelemetry.cs` wired into a real ASP.NET Core app; for Go,
the generated `tracing.go` wired into a real `net/http` server) exports through the generated
...
| `/otel-instrument [lang]` | SDK bootstrap for Node.js / Python / Go, Java agent config, or .NET SDK wiring |
...
`echo`, and `fiber` are refused with that reason today.
