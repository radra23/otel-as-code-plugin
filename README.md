...
Instrument any Node.js, Python, Java, .NET, Go, or Ruby service, infer service identity from your
repo, generate Terraform for Grafana, Datadog, New Relic, or Dash0 — all from your editor.
...
**MVP / experimental.** Scope: Node.js, Python, Java, .NET, Go, and Ruby instrumentation, and
Terraform for four backends (Grafana, Datadog, New Relic, Dash0).
...
app is refused with that reason. **Go** (traces + metrics: Stable; logs: Beta, generated by
default) via a generated `tracing.go` — `go.mod` is edited directly (`go mod tidy` resolves
`go.sum`) and the two wiring lines are printed for you to paste into `main.go`; no edit to your
existing source. `net/http`-compatible servers only (covers `chi`/`gorilla/mux` for free); `gin`,
`echo`, and `fiber` are refused with that reason today. **Ruby** (traces: Stable; metrics and
logs: Development, opt-in via `--experimental`) via `opentelemetry-instrumentation-all` — one
`use_all` call auto-detects and instruments whatever framework is actually present (Rails,
Sinatra, Rack, DB drivers, HTTP clients), so there is no framework refusal case the way Go has
one. `Gemfile` is edited directly (`bundle install` resolves it) and the require line is printed
for non-Rails apps (placed AFTER your framework requires, not before — Rails apps get it
automatically via an auto-loaded initializer). Exports over OTLP/**HTTP** on `:4318`, not this
plugin's usual gRPC `:4317` — Ruby's OTLP gem doesn't support gRPC reliably.
