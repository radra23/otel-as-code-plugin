...
## Scope
Review the changed (or named) files — typically `subagents/instrumentation-gen.md`, `skills/{semconv-discipline,collector-topology,language-maturity,terraform-patterns}.md`, `subagents/{terraform-gen,brownfield-auditor}.md`, `hooks/semconv-lint.sh`.
...
5. **Version pins** — pinned package versions must be real, current release lines, and mutually coherent. `SEMCONV_VERSION` lives ONLY in `skills/semconv-discipline.md`; flag hardcoded version duplicates elsewhere.
